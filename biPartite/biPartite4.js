!function(){
	...
	var colors =["#3366CC", "#DC3912",  "#FF9900","#109618", "#990099", "#0099C6", "#770011", "#141423", "#172278", "#F2C7F1", "#EC9074", "#721053", "#442109", "#821AFC", "#157906", "#342805", "#659431","#B90975", "#C00739", "#295374", "#515469"];
	
	bP.partData1 = function(data,p){
		var sData={};
		
		sData.keys=[
			d3.set(data.map(function(d){ return d["year"];})).values().sort(function(a,b){ return ( a<b? -1 : a>b ? 1 : 0);}),
			d3.set(data.map(function(d){ return d["usages"];})).values().sort(function(a,b){ return ( a<b? -1 : a>b ? 1 : 0);})	
		];
		...
		data.forEach(function(d){ 
			sData.data[0][sData.keys[0].indexOf(d["year"])][sData.keys[1].indexOf(d["usages"])]=d[p];
			sData.data[1][sData.keys[1].indexOf(d["usages"])][sData.keys[0].indexOf(d["year"])]=d[p];
		});
		
		return sData;
	}
	bP.partData2 = function(data,p){
		var sData={};
		
		sData.keys=[
			d3.set(data.map(function(d){ return d["industry"];})).values().sort(function(a,b){ return ( a<b? -1 : a>b ? 1 : 0);}),
			d3.set(data.map(function(d){ return d["usages"];})).values().sort(function(a,b){ return ( a<b? -1 : a>b ? 1 : 0);})	
		];
		...
		data.forEach(function(d){ 
			sData.data[0][sData.keys[0].indexOf(d["industry"])][sData.keys[1].indexOf(d["usages"])]=d[p];
			sData.data[1][sData.keys[1].indexOf(d["usages"])][sData.keys[0].indexOf(d["industry"])]=d[p]; 
		});
		
		return sData;
	}
	bP.partData3 = function(data,p){
		var sData={};
		
		sData.keys=[
			d3.set(data.map(function(d){ return d["industry"];})).values().sort(function(a,b){ return ( a<b? -1 : a>b ? 1 : 0);}),
			d3.set(data.map(function(d){ return d["types"];})).values().sort(function(a,b){ return ( a<b? -1 : a>b ? 1 : 0);})	
		];
		...
		data.forEach(function(d){ 
			sData.data[0][sData.keys[0].indexOf(d["industry"])][sData.keys[1].indexOf(d["types"])]=d[p];
			sData.data[1][sData.keys[1].indexOf(d["types"])][sData.keys[0].indexOf(d["industry"])]=d[p];
		});
	...
	function drawEdges(data, id){
		//console.log(d3.select("#"+id).append("g").attr("class","edges").attr("transform","translate("+ b+",0)"));
		d3.select("#"+id).append("g").attr("class","edges").attr("transform","translate("+ b+",0)");
		//console.log(d3.select("#"+id).select(".edges").selectAll(".edge"));
	...
	function drawHeader(header, id){
		...
		[0,1].forEach(function(d){
			...
			h.append("text").text(header[d]).attr("x",-3)
	...
	bP.draw = function(data, svg, tooltip){
		//console.log(data);
		data.forEach(function(biP,s){
			svg.append("g")
				.attr("id", biP.id)
				.attr("transform","translate("+ (300*s+s*50)+",0)");
			...
			[0,1].forEach(function(p){			
				d3.select("#"+biP.id)
					...
					.on("mouseover",function(d, i){ 
							tooltip.html(biP.data.keys[p][i] + "<br />" + "Amount: " + d.value + "<br />" + "Percent: " + Math.round(d.percent * 100) + "%")
					...
					.on("mouseout",function(d, i){ tooltip.style("opacity", 0.0); return bP.deSelectSegment(data, p, i); });	
			});
			//console.log(data);
		});	
	}
